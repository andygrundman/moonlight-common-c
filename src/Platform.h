...
// Capture the initial timestamp used by the GetTicks64 functions.
void Plt_TicksInit(void);

// The most precision, in microseconds
uint64_t Plt_GetTicks64_us(void);

// Plt_GetTicks64_ms returns the value in milliseconds
uint64_t Plt_GetTicks64_ms(void);

// Plt_GetTicks64 is an alias to _ms() and also the same as SDL_GetTicks64 (milliseconds since program start).
uint64_t Plt_GetTicks64(void);
