...
static PRTPA_FEC_BLOCK getFecBlockForRtpPacket(PRTP_AUDIO_QUEUE queue, PRTP_PACKET packet, uint16_t length) {
    ...
    block->queueTimeUs = Plt_GetTicks64_us();
...
static void handleMissingPackets(PRTP_AUDIO_QUEUE queue) {
    ...
    if (!queue->receivedOosData || Plt_GetTicks64_us() - queue->blockHead->queueTimeUs > (uint64_t)(AudioPacketDuration * RTPA_DATA_SHARDS) + (RTPQ_OOS_WAIT_TIME_MS * 1000)) {
